...
class PPO(PolicyGradient):
    ...
    def clip(self, r_theta, lower, upper):
        return torch.min(torch.max(r_theta, lower),upper)
    ...
    def update_policy(self, observations, actions, advantages, old_logprobs):
        ...
        # print(log_probs)
        r_theta = torch.exp(log_probs-old_logprobs)
        loss = -torch.mean(torch.min(r_theta*advantages,self.clip(r_theta, torch.tensor(1-self.eps_clip), torch.tensor(1+self.eps_clip))*advantages))
        loss.backward()
        self.optimizer.step()
