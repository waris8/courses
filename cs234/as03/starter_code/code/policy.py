...
class BasePolicy:
    ...
    def act(self, observations, return_log_prob = False):
        ...
        if return_log_prob:
            log_probs_t = distribution.log_prob(sampled_actions)
            log_probs = log_probs_t.detach().numpy()
...
class CategoricalPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        distribution = ptd.Categorical(None, self.network(observations))
        #######################################################
        #########          END YOUR CODE.          ############
        return distribution
...
class GaussianPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        distribution = ptd.MultivariateNormal(mean, cov_matrix)
        #######################################################
        #########          END YOUR CODE.          ############
        return distribution
