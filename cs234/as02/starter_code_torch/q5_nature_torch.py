...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        self.q_network = nn.Sequential(nn.Conv2d(n_channels*self.config.state_history, 16, 3),
                                       nn.ReLU(),
                                       nn.Flatten(),
                                       nn.Linear((img_height-2)*(img_height-2)*16, 128),
                                       nn.ReLU(),
                                       nn.Linear(128,num_actions))
        self.target_network = nn.Sequential(nn.Conv2d(n_channels*self.config.state_history, 16, 3),
                                       nn.ReLU(),
                                       nn.Flatten(),
                                       nn.Linear((img_height-2)*(img_height-2)*16, 128),
                                       nn.ReLU(),
    ...
    def get_q_values(self, state, network):
        ...
        inp = state.permute(0,3,1,2)
        if network == "q_network" :
            
            out = self.q_network(inp)
        else :
            out = self.target_network(inp)
        ##############################################################
        ######################## END YOUR CODE #######################
        return out
