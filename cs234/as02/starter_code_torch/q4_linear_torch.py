...
class Linear(DQN):
    ...
    def get_q_values(self, state: torch.Tensor, network: str = "q_network"):
        ...
        if network=="q_network":
            ...
            out = self.q_network(flattened)
        else:
            flatten = torch.nn.Flatten()
            flattened = flatten(state)
            out = self.target_network(flattened)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def calc_loss(
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        bt = torch.bitwise_not(done_mask).to(torch.int)
        
        m,_ = torch.max(target_q_values,1)
        q_sample = rewards + gamma * m * bt
        acs = torch.tensor(actions, dtype=torch.long)
        qs = torch.zeros(32)
        for i in range(32):
            qs[i] = q_values[i][acs[i]]
        loss = torch.nn.functional.mse_loss(q_sample, qs)
        return loss
        ##############################################################
        ######################## END YOUR CODE #######################
